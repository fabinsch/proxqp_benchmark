...
class PROXQPSolver(object):
    ...
    def solve(self, example, n_average, eps):
        ...
        for i in range(n_average):
            ...
            run_time.append(Qp.results.info.run_time)
        # duration time
        std = 1.e-6*np.std(np.array(run_time))
        # print(f"STD: {}")
        # run_time /= (1.e6*n_average) # the run_time is measured in microseconds 
        run_time = 1.e-6*np.mean(np.array(run_time)) # the run_time is measured in microseconds 
...
class PROXQPSolverSparse(object):
